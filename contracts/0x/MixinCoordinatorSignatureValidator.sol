...
contract MixinCoordinatorSignatureValidator is
    ICoordinatorSignatureValidator
{
        ...
        returns (address signerAddress)
    {
        ...
        if (signatureTypeRaw >= uint8(CoordinatorSignatureType.NSignatureTypes)) {
            LibRichErrors.rrevert(LibCoordinatorRichErrors.SignatureError(
        ...
        CoordinatorSignatureType signatureType = CoordinatorSignatureType(signatureTypeRaw);
        ...
        if (signatureType == CoordinatorSignatureType.Illegal) {
            LibRichErrors.rrevert(LibCoordinatorRichErrors.SignatureError(
        ...
        } else if (signatureType == CoordinatorSignatureType.Invalid) {
            LibRichErrors.rrevert(LibCoordinatorRichErrors.SignatureError(
        ...
        } else if (signatureType == CoordinatorSignatureType.EthSign) {
            if (signatureLength != 66) {
                LibRichErrors.rrevert(LibCoordinatorRichErrors.SignatureError(
                    LibCoordinatorRichErrors.SignatureErrorCodes.INVALID_LENGTH,
                    hash,
